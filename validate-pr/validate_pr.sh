...

{
    ...
    if [[ "$HEAD_BRANCH" =~ $FB_PATTERN ]]; then
        if [ "$BASE_BRANCH" != "develop" ]; then
            echo "**ERROR**: A pull request from \`${HEAD_BRANCH}\` is expected to target \`develop\`, not \`${BASE_BRANCH}\`"
    ...
    # 'XX.Y_fb_*' should target 'releaseXX.Y-SNAPSHOT'
        ...
        if [ "$BASE_BRANCH" != "$expected_branch" ]; then
            echo "**ERROR**: A pull request from \`${HEAD_BRANCH}\` is expected to target \`${expected_branch}\`, not \`${BASE_BRANCH}\`"
        fi
    ...
    # Warn about non-standard branch targeting a release branch
        ...
        version=${BASH_REMATCH[1]}
        echo "**ERROR**: Branch doesn't match LabKey naming scheme: \`${HEAD_BRANCH}\`"
...
if [[ "$PR_TITLE" =~ $DEFAULT_TITLE ]]; then
    ...
    echo "**WARNING**: This PR appears to have the default title generated by GitHub. Please use something more descriptive." >>"$ERROR_FILE"
fi
